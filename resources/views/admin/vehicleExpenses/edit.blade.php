        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.vehicleExpense.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route('admin.vehicle-expenses.update', [$vehicleExpense->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="date" id="date" value="{{ old('date', $vehicleExpense->date) }}" required>
                            @if($errors->has('date'))
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="description" id="description">{!! old('description', $vehicleExpense->description) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('payment_reference') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="payment_reference" id="payment_reference" value="{{ old('payment_reference', $vehicleExpense->payment_reference) }}">
                        ...
                        <div class="form-group {{ $errors->has('is_paid') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="is_paid" id="is_paid" value="1" {{ old('is_paid', $vehicleExpense->is_paid ? 1 : 0) == 1 ? 'checked' : '' }}>
                            ...
                            @endif
                            @if($vehicleExpense->paid_at)
                                <span class="help-block">Pago em {{ $vehicleExpense->paid_at->format('Y-m-d H:i:s') }}</span>
                            @endif
                        </div>
                        <div class="form-group {{ $errors->has('value') ? 'has-error' : '' }}">
                            <label class="required" for="value">{{ trans('cruds.vehicleExpense.fields.value') }}</label>
                            <input class="form-control" type="number" name="value" id="value" value="{{ old('value', $vehicleExpense->value) }}" step="0.01" required>
                            @if($errors->has('value'))
                        ...
                        <div class="form-group {{ $errors->has('vat') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="vat" id="vat" value="{{ old('vat', $vehicleExpense->vat) }}" step="0.01" required>
                            @if($errors->has('vat'))
