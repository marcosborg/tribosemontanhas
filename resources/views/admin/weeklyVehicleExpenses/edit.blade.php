        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.weeklyVehicleExpense.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.weekly-vehicle-expenses.update", [$weeklyVehicleExpense->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('vehicle_item') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="vehicle_item_id" id="vehicle_item_id" required>
                                @foreach($vehicle_items as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('vehicle_item_id') ? old('vehicle_item_id') : $weeklyVehicleExpense->vehicle_item->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('driver') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="driver_id" id="driver_id" required>
                                @foreach($drivers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('driver_id') ? old('driver_id') : $weeklyVehicleExpense->driver->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('tvde_week') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="tvde_week_id" id="tvde_week_id" required>
                                @foreach($tvde_weeks as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('tvde_week_id') ? old('tvde_week_id') : $weeklyVehicleExpense->tvde_week->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('total_km') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="total_km" id="total_km" value="{{ old('total_km', $weeklyVehicleExpense->total_km) }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('weekly_km') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="weekly_km" id="weekly_km" value="{{ old('weekly_km', $weeklyVehicleExpense->weekly_km) }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('extra_km') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="extra_km" id="extra_km" value="{{ old('extra_km', $weeklyVehicleExpense->extra_km) }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('transfers') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="transfers" id="transfers" value="{{ old('transfers', $weeklyVehicleExpense->transfers) }}" step="0.01">
                        ...
                        <div class="form-group {{ $errors->has('deposit') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="deposit" id="deposit" value="{{ old('deposit', $weeklyVehicleExpense->deposit) }}" step="0.01">
