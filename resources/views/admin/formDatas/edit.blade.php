                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.form-datas.update", [$formData->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('form_name') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="form_name_id" id="form_name_id" required>
                                @foreach($form_names as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('form_name_id') ? old('form_name_id') : $formData->form_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('driver') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="driver_id" id="driver_id">
                                @foreach($drivers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('driver_id') ? old('driver_id') : $formData->driver->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('vehicle_item') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="vehicle_item_id" id="vehicle_item_id">
                                @foreach($vehicle_items as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('vehicle_item_id') ? old('vehicle_item_id') : $formData->vehicle_item->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('user') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="user_id" id="user_id">
                                @foreach($users as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $formData->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('user'))
                        ...
                        <div class="form-group {{ $errors->has('data') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="data" id="data" required>{{ old('data', $formData->data) }}</textarea>
                            @if($errors->has('data'))
                        ...
                        <div class="form-group {{ $errors->has('solved') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="solved" id="solved" value="1" {{ $formData->solved || old('solved', 0) === 1 ? 'checked' : '' }}>
