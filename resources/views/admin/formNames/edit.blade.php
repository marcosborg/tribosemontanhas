        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.formName.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.form-names.update", [$formName->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.formName.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $formName->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="description" id="description">{!! old('description', $formName->description) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('has_driver') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="has_driver" id="has_driver" value="1" {{ $formName->has_driver || old('has_driver', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('has_license') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="has_license" id="has_license" value="1" {{ $formName->has_license || old('has_license', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('roles') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="roles[]" id="roles" multiple>
                                @foreach($roles as $id => $role)
                                    <option value="{{ $id }}" {{ (in_array($id, old('roles', [])) || $formName->roles->contains($id)) ? 'selected' : '' }}>{{ $role }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('roles'))
