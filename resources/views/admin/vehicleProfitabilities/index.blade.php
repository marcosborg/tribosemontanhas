...
<div class="content">
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                        ...
                        <div class="form-group period-block period-week" style="margin-right:8px; display:none; min-width:280px;">
                            ...
                            <select name="weeks[]" id="weeks" class="form-control" multiple size="4" style="min-width:260px;">
                                @foreach($tvde_weeks as $w)
                                    <option value="{{ $w->id }}" {{ collect(request('weeks', []))->contains($w->id) ? 'selected' : '' }}>
                    ...
                    <ul class="nav nav-tabs" style="margin-top:15px;">
                        <li role="presentation" {{ $isGlobal ? 'class=active' : '' }}>
                            <a href="/admin/vehicle-profitabilities/set-vehicle-item-id/global">Global</a>
                        </li>
                        @foreach ($vehicle_items as $vi)
                            <li role="presentation" {{ !$isGlobal && (int) $vehicle_item_id === (int) $vi->id ? 'class=active' : '' }}>
            ...
            @if($isGlobal)
                <div class="panel panel-default">
                    <div class="panel-heading">Resultado Final por Viatura</div>
                    <div class="panel-body" style="height:420px;">
                        <canvas id="globalRentChart"></canvas>
                    </div>
                </div>

                <div class="panel panel-default">
                    <div class="panel-heading">Totais por Viatura</div>
                        ...
                        <table class="table table-bordered table-striped table-sm">
                            ...
                            <tbody>
                                @forelse($globalRows as $row)
                                    <tr>
                                        <td>{{ $row['vehicle_item']->license_plate }}</td>
                                        <td>{{ $row['vehicle_item']->driver->name ?? '—' }}</td>
                                        <td>{{ number_format($row['treasury'], 2, ',', '.') }} €</td>
                                        <td>{{ number_format($row['taxes'], 2, ',', '.') }} €</td>
                                        <td><strong>{{ number_format($row['final'], 2, ',', '.') }} €</strong></td>
                                    </tr>
                                @empty
                                ...
                                @forelse($rows as $r)
                                    <tr>
                                        ...
                                        <td style="min-width:260px;">
                                            @php $items = $r['vehicle_expenses_items'] ?? []; @endphp
                                                                ...
                                                                <tbody>
                                                                    @foreach($items as $e)
                                                                        <tr>
                                                                            ...
                                                                            <td>
                                                                                {{ $e['expense_type'] ?? '—' }}
                                                                                @php $norm = $e['effective_normalized_type'] ?? $e['normalized_type'] ?? null; @endphp
                                                                                @if(!empty($norm))
                                                                                    <br><small style="color:#666;">({{ $norm }})</small>
                                                                                @endif
                                                                            </td>
                                                                            <td>{{ number_format($e['value'] ?? 0, 2, ',', '.') }} €</td>
                                                                            <td>{{ $e['invoice_value'] !== null ? number_format($e['invoice_value'], 2, ',', '.') . ' €' : '—' }}</td>
...
<script>
    ...
    @if($isGlobal)
        const globalChartCanvas = document.getElementById('globalRentChart');

        if (globalChartCanvas) {
            new Chart(globalChartCanvas.getContext('2d'), {
                type: 'bar',
                data: {
                    labels: @json($globalChart['labels']),
                    datasets: [{
                        label: 'Resultado Final (EUR)',
                        data: @json($globalChart['final']),
                        borderWidth: 1,
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'Rentabilidade Global por Viatura'
                        }
                    },
                    scales: {
                        y: {
