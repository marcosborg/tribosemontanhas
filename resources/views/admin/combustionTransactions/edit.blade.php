        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.combustionTransaction.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.combustion-transactions.update", [$combustionTransaction->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('tvde_week') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="tvde_week_id" id="tvde_week_id" required>
                                @foreach($tvde_weeks as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('tvde_week_id') ? old('tvde_week_id') : $combustionTransaction->tvde_week->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('card') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="card" id="card" value="{{ old('card', $combustionTransaction->card) }}" required>
                            @if($errors->has('card'))
                        ...
                        <div class="form-group {{ $errors->has('total') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="total" id="total" value="{{ old('total', $combustionTransaction->total) }}" step="0.01" required>
                            @if($errors->has('total'))
                        ...
                        <div class="form-group {{ $errors->has('transaction_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="datetime-local" name="transaction_date" id="transaction_date" value="{{ old('transaction_date', optional($combustionTransaction->transaction_date)->format('Y-m-d\\TH:i:s')) }}">
